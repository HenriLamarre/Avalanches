...
class Farhang:
    ...
    def __init__(self, d, n):
        ...
        self.last_time = 0
        self.curvs = []
        self.shuffle_x = np.arange(0, self.N)
        self.shuffle_y = np.arange(0, self.N)
    ...
    def step(self, step_time):
        ...
        np.random.shuffle(self.shuffle_x)
        np.random.shuffle(self.shuffle_y)
        for i in self.shuffle_x:  # loops over lattice to check if avalanche should occur
            for j in self.shuffle_y:
                if curv[i][j] > Zc:  # condition to investigate optimization
                    ...
                    if deltaE < 0:  # If there should be an avalanche
                        e -= deltaE
                        self.lat_B[i, j] -= 4 / 5 * Zc  # Updates the avalanche lattice array
                        self.lat_B[seq[0][0], seq[0][1]] += 4 / 5 * r1 / (x + a) * Zc
                        self.lat_B[seq[2][0], seq[2][1]] += 4 / 5 * r3 / (x + a) * Zc
                        self.lat_B[seq[3][0], seq[3][1]] += 4 / 5 * x / (x + a) * Zc
                        self.lat_B[seq[1][0], seq[1][1]] += 4 / 5 * r2 / (x + a) * Zc
    ...
    def loop(self, total_time, save=False, load=False):
        ...
        if load:  # Otherwise, if we have an initial array
            self.lat_B = np.load(load + 'N{}_Farhang_shuffle.npz'.format(self.N))['lat_B']
        else:
        ...
        for i in range(int(total_time)):  # The actual loop
            if not i % int(total_time/100):  # Progress bar
                ...
                if save:  # saves the array periodically
                    np.savez(save + 'N{}_Farhang_shuffle.npz'.format(self.N), lat_B=self.lat_B)
            self.step(i)
