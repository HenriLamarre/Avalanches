                ...
                if i == 0 or i ==self.n-1 or j == 0 or j == self.n-1:
                    ...
                    if curv[i, j] > Zc:
                        ...
                        for k in range(20):
                            ...
                            new_energy_dissipated = -np.sum(np.multiply(energy_calc_lattice, energy_calc_lattice)) +\
                                                np.sum(np.multiply(self.lat_B, self.lat_B))
                            if new_energy_dissipated > energy_dissipated:
                                energy_dissipated = new_energy_dissipated
                                rs_saved = rs
                        if not energy_dissipated:
                            has_reached_positive_energy = False
                        if has_reached_positive_energy:
                            a = np.sum(rs)
                            self.lat_C[i, j] -= 4/5*Zc
                            self.lat_C[i+1, j] += 4 / 5 * Zc * rs_saved[0] / a
                            self.lat_C[i-1, j] += 4 / 5 * Zc * rs_saved[1] / a
                            self.lat_C[i, j+1] += 4 / 5 * Zc * rs_saved[2] / a
                            self.lat_C[i, j-1] += 4 / 5 * Zc * rs_saved[3] / a
